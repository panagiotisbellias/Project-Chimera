# =============================================================================
# quant_recency_backtest.py
#
# Description:
#   This script serves as the definitive recency backtest for Project Chimera.
#   It completely replaces the 'yfinance' dependency, sourcing all historical
#   data directly from the Alpaca API. This ensures 100% consistency between
#   the data used for backtesting and the data used in live trading.
#
#   The script fetches historical data, manually calculates all 6 core features
#   required by the Causal Engine, and then runs a 30-day simulation to test
#   the agent's performance on the most recent market conditions.
# =============================================================================
...
print("--- Initializing Recency Backtest with Alpaca Data ---")
load_dotenv()

# --- Backtest Configuration ---
SIMULATION_DAYS = 30  # Test the most recent 30 days
AGENT_MODEL = "gpt-4o"
...
current_state_for_tools = {} # Global state for agent tools
print("✅ Guardian and Causal Engine are ready.")


# =============================================================================
# --- Section 4: Data Preparation using Alpaca ---
# =============================================================================

def prepare_backtest_data(history_days: int = 400):
    """
    (SLOW STRATEGY + ATR VERSION)
    Fetches historical data from Alpaca and manually calculates all core features,
    including the new Average True Range (ATR) for volatility analysis.
    """
...
# =============================================================================
# --- The rest of the script is the familiar backtesting engine ---
# =============================================================================

# --- Agent Tools Definition ---
@tool
def check_action_validity(action_type: str, amount: float) -> str:
    """Checks if a proposed trading action is valid."""
    action = {'type': action_type.upper(), 'amount': amount}
    # Note: The backtest simulator state is slightly different from the live state.
    # We create a compatible dictionary for the guardian.
    state_for_guardian = {
        "cash": current_state_for_tools.get('cash'),
        "shares_held": current_state_for_tools.get('shares_held'),
        "market_data": current_state_for_tools.get('market_data')
    }
...
def analyze_and_report(history_df, actions_df, market_data_df, initial_capital):
    """Generates and saves a performance report dashboard."""
    # This function should be copied from your original 'quant_run_backtest.py'
    # Ensure you change the output filename to 'recency_backtest_report.png'
    print("\n--- Generating Recency Test Performance Report ---")
    # Placeholder for your detailed plotting function
    final_value = history_df['portfolio_value'].iloc[-1]
    total_return = (final_value / initial_capital) - 1
    print(f"Final Portfolio Value: ${final_value:,.2f}")
    print(f"Total Return: {total_return:.2%}")
    # --- Add your full plotting logic here ---

def print_daily_report(day, sim_state_before, sim_state_after, agent_response, action_data, total_days, initial_capital):
    """
    Prints a beautifully formatted, detailed summary of the agent's decision,
    including both daily and total portfolio P&L.
    """
    
    # --- 1. Extract Data ---
    market_data = sim_state_after.get('market_data', {})
    value_before = sim_state_before.get('portfolio_value', 0)
    value_after = sim_state_after.get('portfolio_value', 0)
    cash_after = sim_state_after.get('cash', 0)
    shares_after = sim_state_after.get('shares_held', 0)
    commentary = action_data.get('commentary', 'No commentary provided.')
    action = action_data.get('action', {})
    action_type = action.get('type', 'N/A').upper()
    action_amount = action.get('amount', 0.0)

    # --- 2. Calculate Daily & TOTAL Performance ---
    daily_pnl = value_after - value_before
    daily_pnl_pct = (daily_pnl / value_before) if value_before != 0 else 0.0
    
    total_pnl = value_after - initial_capital
    ...
    action_icon = "✅" if action_type in ['BUY', 'SHORT'] else "❌" if action_type == 'SELL' else "ℹ️"
    
    # --- 4. Build and Print the Report ---
    import textwrap
    header_text = f"| {HEADER}🚀 CHIMERA DAILY REPORT | DAY {day+1}/{total_days}{ENDC}"
    print("\n" + "="*80)
    print(header_text.ljust(99) + "|")
    print("="*80)
    print("|")
    price_str = f"${market_data.get('Close', 0):,.2f}".ljust(15)
    rsi_str = f"{market_data.get('Rsi_14', 0):.2f}".ljust(6)
    macd_str = f"{market_data.get('Macdh_12_26_9', 0):.2f}"
    print(f"|   {BOLD}📊 MARKET SNAPSHOT{ENDC}")
    print(f"|   {'------------------'}")
    print(f"|   BTC Price: {CYAN}{price_str}{ENDC}|  RSI: {CYAN}{rsi_str}{ENDC}|  MACD Hist: {CYAN}{macd_str}{ENDC}")
    print("|")
    print(f"|   {BOLD}💡 AGENT DECISION{ENDC}")
    print(f"|   {'-----------------'}")
    wrapped_commentary = textwrap.fill(commentary, width=70).replace('\n', '\n|               ')
    print(f"|   Commentary: {wrapped_commentary}")
    print(f"|   Action:     {action_color}{action_icon} {action_type} {action_amount:.2%} of available assets.{ENDC}")
    print("|")
    value_change_str = f"${value_before:,.2f} -> ${value_after:,.2f}"
    pnl_str = f"{pnl_color}{daily_pnl:+.2f} ({daily_pnl_pct:+.2%}) {pnl_icon}{ENDC}"
    # !!! YENİ EKLENEN SATIR !!!
    total_pnl_str = f"{total_pnl_color}{total_pnl:+.2f} ({total_pnl_pct:+.2%}) {total_pnl_icon}{ENDC}"
    print(f"|   {BOLD}💼 PORTFOLIO IMPACT{ENDC}")
    print(f"|   {'--------------------'}")
    print(f"|   Value Change:    {YELLOW}{value_change_str}{ENDC}")
    print(f"|   Daily P&L:       {pnl_str}")
    print(f"|   Total P&L:       {total_pnl_str}") # YENİ SATIR BURADA
    print("|")
    print("|   Final Status:")
    print(f"|   - Cash:          {YELLOW}${cash_after:,.2f}{ENDC}")
    print(f"|   - Shares:        {YELLOW}{shares_after:.4f} BTC{ENDC}")
    print("="*80)


# --- Main Execution Block ---
def run_recency_backtest():
    """Main function to orchestrate the 30-day backtest."""
    full_market_data = prepare_backtest_data()
    
    # Select the last 30 days of data for the simulation
    ...
    *Final Output:*
    ...
    print(f"\n--- Starting {SIMULATION_DAYS}-Day Recency Backtest ---")
    simulator = MarketSimulatorV2(market_data=simulation_data, initial_capital=INITIAL_CAPITAL)
    history, actions_log = [], []

    # The main simulation loop
    for day in tqdm(range(len(simulation_data)), desc="Simulating Recent Trading Days"):
        sim_state_before = simulator.get_state()
        history.append(sim_state_before)
        global current_state_for_tools
        current_state_for_tools = sim_state_before # Update global state for tools
...
def analyze_and_report(history_df, actions_df, market_data_df, initial_capital):
    ...
    output_path = os.path.join("results/quant", "recency_test_report.png")
